...
<h3 id="attributeheader">
  ...
  <?php endif; ?>
  <?php
  if (!empty($this->data['noty']['title'])) : ?>
    <p><?= $this->t('{themeopenaire:attrauthcomanage_noty:' . $this->data['noty']['title'] . '}') ?></p>
   <!-- XXX This is a workaround that does not require new configuration. We should fix it eventually. -->
  <?php if (!empty($this->data['noty']['icon'])) : ?>
    <span class="resend-noty-img">
      <img src="/<?= $this->data['baseurlpath'] ?>/module.php/themeopenaire/resources/icons/envelope.svg">
    </span>
  <?php endif; ?>
  <?php
  endif; ?>
...
<div class="noty-description">
  ...
  foreach ($this->data['noty']['description'] as $dictKey => $placeholders) {
    if (!empty($placeholders)) {
      echo $this->t('{themeopenaire:attrauthcomanage_noty:' . $dictKey . '}', $placeholders);
    } else {
      echo $this->t('{themeopenaire:attrauthcomanage_noty:' . $dictKey . '}');
    }
  }
  ?>
</div>
...
<div class="ssp-btns-container">
  ...
  >
    ...
    endforeach; ?>
    <?php
    foreach ($this->data['noty']['form_fields'] as $name => $value) : ?>
      <input
        type="hidden"
        name="<?= htmlspecialchars($name) ?>"
        value="<?= htmlspecialchars($value) ?>"
  ...
  >
    ...
    <button
      ...
      id="nobutton"
    >
      <?= htmlspecialchars($this->t('{themeopenaire:attrauthcomanage_noty:no}')) ?>
    </button>
  </form>
</div>
<?php
...
<script type="application/javascript">
  let button_text = '<?= $this->t('{themeopenaire:attrauthcomanage_noty:yes}') ?>';
  ...
  function form_ajax(frm_ob) {
    ...
    jqxhr.done((data, textStatus, xhr) => {
      ...
      $('.invite-expire')
        .html('<?= htmlspecialchars($this->t('{themeopenaire:attrauthcomanage_noty:invitation_restored}')) ?>');
